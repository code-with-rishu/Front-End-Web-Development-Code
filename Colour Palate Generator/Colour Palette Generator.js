...
function getCategory(name, hexVal) {
  var rgb = hexToRgb(hexVal);
  var r = rgb.r, g = rgb.g, b = rgb.b;
  var l = (Math.max(r, g, b) + Math.min(r, g, b)) / 2;
  var hue = rgbToHsl(r, g, b).h;
  if (l < 0.2 || l > 0.85) return 'neutral';
  var n = name.toLowerCase();
  if (n.indexOf('brown') !== -1 || n.indexOf('sienna') !== -1 || n.indexOf('saddle') !== -1 || n.indexOf('chocolate') !== -1 || n.indexOf('peru') !== -1 || n.indexOf('tan') !== -1 || n.indexOf('burly') !== -1 || n.indexOf('rosy') !== -1 || n.indexOf('sandy') !== -1 || n.indexOf('goldenrod') !== -1 || n.indexOf('gold') !== -1 || n.indexOf('orange') !== -1 || n.indexOf('yellow') !== -1 || n.indexOf('khaki') !== -1 || n.indexOf('wheat') !== -1 || n.indexOf('navajo') !== -1 || n.indexOf('blanched') !== -1 || n.indexOf('bisque') !== -1 || n.indexOf('peach') !== -1 || n.indexOf('lemon') !== -1) return 'warm';
  if (hue >= 180 && hue <= 270) return 'cool';
  if (hue >= 270 || hue < 30) return 'cool';
  return 'warm';
}

function isPrimary(name) {
  var p = ['Red','Blue','Green','Yellow','Cyan','Magenta','White','Black','Gray','Grey','Orange','Purple','Brown','Pink','Silver','Gold'];
...
function renderColors() {
  ...
  var filtered = COLORS.filter(function(c) {
    var matchSearch = c.name.toLowerCase().indexOf(searchTerm.toLowerCase()) !== -1 || c.hex.toLowerCase().indexOf(searchTerm.toLowerCase()) !== -1;
    var matchFilter = currentFilter === 'all' ||
      (currentFilter === 'warm' && getCategory(c.name, c.hex) === 'warm') ||
      (currentFilter === 'cool' && getCategory(c.name, c.hex) === 'cool') ||
      (currentFilter === 'neutral' && getCategory(c.name, c.hex) === 'neutral') ||
      (currentFilter === 'primary' && isPrimary(c.name));
    return matchSearch && matchFilter;
  });

  if (currentSort === 'name') filtered.sort(function(a, b) { return a.name.localeCompare(b.name); });
  else if (currentSort === 'hex') filtered.sort(function(a, b) { return a.hex.localeCompare(b.hex); });
  else if (currentSort === 'hue') filtered.sort(function(a, b) { return rgbToHsl(hexToRgb(a.hex).r, hexToRgb(a.hex).g, hexToRgb(a.hex).b).h - rgbToHsl(hexToRgb(b.hex).r, hexToRgb(b.hex).g, rgbToHsl(b.hex).b).h; });

  stats.textContent = 'Showing ' + filtered.length + ' of ' + COLORS.length + ' colors';

  if (filtered.length === 0) {
    grid.innerHTML = '<div class="no-results">No colors found matching your search</div>';
    return;
  }
  ...
  filtered.forEach(function(c) {
    ...
    html += '<div class="color-card">' +
      '<div class="color-swatch" style="background:' + c.hex + '">' +
        '<button class="copy-btn" onclick="copyCode(\'' + c.hex + '\', event)">Copy Hex</button>' +
      '</div>' +
      '<div class="color-info">' +
        '<div class="color-name">' + c.name + '</div>' +
        '<div class="code-row">' +
          '<span class="code-label">#</span>' +
          '<span class="code-value" onclick="copyCode(\'' + c.hex + '\', event)" style="color:' + textColor + '">' + c.hex + '</span>' +
        '</div>' +
        '<div class="code-row">' +
          '<span class="code-label">RGB</span>' +
          '<span class="code-value" onclick="copyCode(\'' + rgbStr + '\', event)" style="color:' + textColor + '">' + rgbStr + '</span>' +
        '</div>' +
        '<div class="code-row">' +
          '<span class="code-label">HSL</span>' +
          '<span class="code-value" onclick="copyCode(\'' + hslStr + '\', event)" style="color:' + textColor + '">' + hslStr + '</span>' +
        '</div>' +
      '</div>' +
    '</div>';
  });
